...
\[ \not \exists \lim_{x \to a} f(x) \Leftrightarrow \exists \epx > 0\; \forall \delta\; \exists x`, x``\; 0 < |x` - a| < \delta \;\; 0 < |x`` - a| < \delta |f(x`) - f(x``)| \geq \eps \]
\textbf{Пример. } 
\begin{enumerate}
...
\begin{enumerate}
	\item $ f(x) \ to \infty, x \to a \Leftrightarrow \forall \eps > 0\; \exists \delta_\eps > 0: \forall x\; 0< |x - a| < \delta_\eps \;\; |f(x)| > \eps \Leftrightarrow f(x) \in U_{\infyu, \eps}$
