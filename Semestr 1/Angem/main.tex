...
\[
	...
	Канонические уравнения L.
		\item \[ \overline{M_0M} = t \overline{q} \]
			...
			\end{cases} $
		\item $ \begin{cases}
			\text{П1: }A_1x + B_1y + C_1z + D_1 = 0\\
			\text{П2: }A_2x + B_2y + C_2z + D_2 = 0
		\end{cases} $
...
\[+ 2D(x` \cos \alpha - y` \sin\alpha) + 2E(x` \sin \alpha + y` \cos \alpha) + F = 0\]
...
\begin{enumerate}
	...
	\item Однополостный гиперболоид
		...
		\end{center}
			\[ \frac{x^2}{a^2} + \frac{y^2}{b^2} - \frac{z^2}{c^2} = 1 \]
			\textbf{Змечание.} Знак - может стоять перед любой дробью. Не обязателно перед z.
...
\begin{tcolorbox}
\textbf{Определение.} Пусть $A = (a_{ij})_{m\times n}$, $B = (b_{ij})_{m\times n}$ — матрицы одинакового размера.  \\
\textit{Суммой} матриц $A + B$ называется матрица $C = (c_{ij})_{m\times n}$, где
\[
...
\begin{tcolorbox}
\textbf{Определение.} Произведением матрицы $A = (a_{ij})_{m\times n}$ на число $\lambda \in \R$ называется матрица $\lambda A = (\lambda a_{ij})_{m\times n}$.
\end{tcolorbox}

\begin{tcolorbox}[title=Пример умножения матриц]
Рассмотрим матрицы
\[
A = \begin{pmatrix}
1 & 2 \\
...
\end{pmatrix}.
\]
Матрица \(A\) имеет размер \(2 \times 2\), матрица \(B\) — размер \(2 \times 3\).  
Произведение \(AB\) определено, так как число столбцов \(A\) равно числу строк \(B\). Результат будет матрицей размера \(2 \times 3\):

\[
AB =
\begin{pmatrix}
1\cdot2 + 2\cdot1 & 1\cdot0 + 2\cdot4 & 1\cdot(-1) + 2\cdot2 \\
(-1)\cdot2 + 3\cdot1 & (-1)\cdot0 + 3\cdot4 & (-1)\cdot(-1) + 3\cdot2
\end{pmatrix}
=
\begin{pmatrix}
4 & 8 & 3 \\
...
\begin{tcolorbox}
\textbf{Определение.} Пусть $A = (a_{ij})_{m\times p}$, $B = (b_{jk})_{p\times n}$.  
\textit{Произведением} $AB$ называется матрица $C = (c_{ik})_{m\times n}$, где
\[
c_{ik} = \sum_{j=1}^{p} a_{ij} b_{jk}.
\]
\end{tcolorbox}

\begin{tcolorbox}[title=Свойства умножения матриц]
\begin{enumerate}
    \item $(AB)C = A(BC)$ — ассоциативность.
    \item $A(B + C) = AB + AC$, $(A + B)C = AC + BC$ — дистрибутивность.
    \item В общем случае $AB \ne BA$ — некоммутативность.
\end{enumerate}
\end{tcolorbox}

\subsubsection{Транспонирование матрицы}
\begin{tcolorbox}
\textbf{Определение.} Матрица $A^T$, полученная из $A = (a_{ij})_{m\times n}$ заменой строк на столбцы, называется \textit{транспонированной}:
\[
A^T = (a_{ji})_{n\times m}.
\]
\end{tcolorbox}

\begin{tcolorbox}[title=Свойства транспонирования]
\begin{enumerate}
    \item $(A^T)^T = A$,
    \item $(A + B)^T = A^T + B^T$,
    \item $(AB)^T = B^T A^T$.
...
\subsubsection{Определитель квадратной матрицы}
\begin{tcolorbox}
\textbf{Определение.} Пусть $A = (a_{ij})_{n\times n}$. \textit{Определителем} (детерминантом) матрицы $A$ называется число
\[
\det A = \sum_{\sigma \in S_n} \operatorname{sign}(\sigma) \cdot a_{1\sigma(1)} a_{2\sigma(2)} \dots a_{n\sigma(n)},
\]
где сумма берётся по всем перестановкам $\sigma$ множества $\{1,2,\dots,n\}$, а $\operatorname{sign}(\sigma)$ — чётность перестановки.
\end{tcolorbox}

\begin{tcolorbox}[title=Свойства определителя]
\begin{enumerate}
    \item $\det A^T = \det A$,
    \item Если поменять местами две строки (столбца), определитель меняет знак,
    \item Если строка (столбец) состоит из нулей, то $\det A = 0$,
    \item $\det(AB) = \det A \cdot \det B$,
    \item $\det(\lambda A) = \lambda^n \det A$, если $A$ — $n\times n$.
\end{enumerate}
\end{tcolorbox}

\textbf{Мне кажется их не было, но нейронка накидала. Пусть будет}
\begin{tcolorbox}[title=Минор и алгебраическое дополнение]
Пусть $A = (a_{ij})_{n\times n}$.  
\textit{Минором} $M_{ij}$ называется определитель матрицы $(n-1)\times(n-1)$, полученной из $A$ вычёркиванием $i$-й строки и $j$-го столбца.  
\textit{Алгебраическое дополнение}:
\[
A_{ij} = (-1)^{i+j} M_{ij}.
\]
\end{tcolorbox}

\begin{tcolorbox}[title=Разложение определителя по строке (столбцу)]
\[
\det A = \sum_{j=1}^{n} a_{ij} A_{ij} \quad\text{(по $i$-й строке)},
\]
\[
\det A = \sum_{i=1}^{n} a_{ij} A_{ij} \quad\text{(по $j$-му столбцу)}.
\]
\end{tcolorbox}

\subsubsection{Обратная матрица}
\begin{tcolorbox}
\textbf{Определение.} Матрица $A^{-1}$ называется \textit{обратной} к $A$, если
\[
AA^{-1} = A^{-1}A = I,
\]
...
\begin{tcolorbox}[title=Условие существования обратной матрицы]
Квадратная матрица $A$ обратима тогда и только тогда, когда $\det A \ne 0$.
\end{tcolorbox}

\begin{tcolorbox}[title=Формула обратной матрицы]
\[
A^{-1} = \frac{1}{\det A} \cdot \bigl( A_{ji} \bigr)_{n\times n} = \frac{1}{\det A} \cdot (\operatorname{adj} A)^T,
\]
где $\operatorname{adj} A$ — матрица алгебраических дополнений.
\end{tcolorbox}
